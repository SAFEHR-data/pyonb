...
async def inference() -> dict:
    ...
    for filename in filenames:
        with Path.open(Path(DATA_FOLDER) / Path(filename), "rb") as pdf_file:
            # Send the file via POST request
            s1 = datetime.datetime.now(datetime.UTC)
            response = requests.post(url, files={"file": (str(filename), pdf_file, "application/pdf")}, timeout=5)  # noqa: ASYNC210
