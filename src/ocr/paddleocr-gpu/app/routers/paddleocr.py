...
@lru_cache(maxsize=1)
def load_paddle_model(ocr_version, lang):
    return PaddleOCR(ocr_version=ocr_version, lang=lang, use_angle_cls=True, enable_mkldnn=True)
...
def invoke_ocr(doc, file_name, file_content_type, page_num, ocr_version, lang):
    ...
    model = load_paddle_model(ocr_version=ocr_version, lang=lang)

    bytes_img = io.BytesIO()

    logger.debug(f"content_type: {file_name}")
    format_img = "JPEG"
...
async def inference(file: UploadFile = File(None),
                    ocr_version: Annotated[str, Form()] = "PP-OCRv3",
